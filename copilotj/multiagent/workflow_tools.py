...
async def batch_precheck_workflow(
    workflow_id: Annotated[str, "The ID of the workflow to pre-check"],
    data_path: Annotated[str, "Path to the data folder to check"],
    sample_size: Annotated[int, "Number of images to sample for QC (default: 9)"] = 9,
) -> str:
    try:
        from copilotj.multiagent.batch_qc import batch_precheck

        report = await batch_precheck(
            folder_path=data_path,
            sample_size=sample_size,
        )

        workflow = WorkflowManager.load_workflow(workflow_id)
        workflow_name = workflow.meta.name if workflow else workflow_id

        return f"""## Pre-Batch QC Report for Workflow: {workflow_name}
...
**Next Steps**:
- Review the QC report above
- If issues are detected, consider adjusting the workflow or data preprocessing
- Run `execute_workflow` when ready to proceed with batch processing
"""

    except Exception as e:
        return f"❌ Error running batch pre-check: {str(e)}"
